...
#Set permission to insert data
# sqlQuery(db_context, 'SET IDENTITY_INSERT Participant ON')
# sqlQuery(db_context, 'SET IDENTITY_INSERT ParticipantRating ON')
# sqlQuery(db_context, 'SET IDENTITY_INSERT Rating ON')
# sqlQuery(db_context, 'SET IDENTITY_INSERT Movie ON')
```
...
```{r sql-insert-commands, eval = F}
#Insert data, run and comment out (cannot duplicate data)sqlQuery(db_context, 'INSERT INTO Participant VALUES(\'Layla\', \'Quinones\'), (\'Daniel\', \'Rivera\'),  (\'London\', \'Ferrer\'), (\'Rose\', \'Burnell\');')
#sqlQuery(db_context, 'INSERT INTO Participant VALUES (\'John\', \'Smith\'), (\'Jane\', \'Smith\'), (\'Angie\', \'Abreu\'), (\'Amy\', \'Thompson\'), (\'Denys\', \'Michell\');')
...
#knitr::include_graphics("DB.jpg")
```


```{r set-demo-variables}
(sqlQuery(db_context, 'select * from Participant'))
(sqlQuery(db_context, 'select * from Rating'))
(movie <- sqlQuery(db_context, 'select * from Movie'))
(sqlQuery(db_context, 'SELECT TOP 10 * FROM ParticipantRating'))
```
...
medianRat
boxplot(ratingData$RatingID ~ ratingData$MovieID, main = "Participant Ratings", xlab = "Movie ID", ylab = "Rating")

barplot(medianRat ~ c(1, 2, 3, 4, 5, 6), beside=TRUE,col = "purple", main = "Median Participant Ratings", xlab = "Movie ID", ylab = "Median Rating")
```
