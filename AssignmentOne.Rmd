...
## Data Wrangling
I found the column names to be great, so no renaming of columns.

```{r import, message=F}
data_hate_crimes <- read_csv(
...
ggplot(data_hate_crimes, aes(median_household_income, hate_crimes_per_100k_splc)) +
    geom_smooth()
...
ggplot(data_hate_crimes, aes(share_voters_voted_trump, hate_crimes_per_100k_splc)) +
    geom_smooth()
```
...
data_red_blue <- left_join(data_hate_crimes, data_red_blue, by ="state") %>%
    select(state, party, hate_crimes_per_100k_splc, median_household_income, 
           share_voters_voted_trump, share_population_in_metro_areas)

ggplot(data_red_blue, aes(share_voters_voted_trump, hate_crimes_per_100k_splc)) +
    geom_smooth() + geom_point(aes(color = party)) +
    scale_color_manual(values=c("blue","red"))

ggplot(data_red_blue, aes(median_household_income, hate_crimes_per_100k_splc)) +
    geom_smooth() + geom_point(aes(color = party)) +
    scale_color_manual(values=c("blue","red"))
```
